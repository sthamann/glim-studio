...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            Text("Below 48 GB, the app uses the compact model. Below 32 GB, it starts with small drafts and releases cached models after saving each image. The next image may take longer to start. MacBook performance has not yet been measured.").font(.caption).foregroundStyle(.secondary)
            HStack {
            ...
            Text("Glim Studio \(Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "") · Apple Silicon · macOS 14+").font(.caption2).foregroundStyle(.tertiary)
        }
    }
}
